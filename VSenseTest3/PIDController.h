...
void configPulseWidth(double val){
  TimeBase=val;
  AutoPID myPIDNew(&temperature, &setPoint, &outputVal, OUTPUT_MIN, OUTPUT_MAX, KP, KI, KD);
  myPID=myPIDNew;
}
...
bool updateTemperature() {
  ...
  if ((millis() - lastTempUpdate) > TEMP_READ_DELAY) {
    ...
    Serial.print("volts");
    delay(200);
    float v=map(ADC_VALUE,0,4095,10,40);

    temperature = v;//getTempValue();
...
void loopPID() {
  ...
  changeRelayState(myPID.atSetPoint(1));//when we're at setpoint +-1 degree
