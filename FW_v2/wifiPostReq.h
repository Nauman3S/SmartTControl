...
const char *serverNameV = "http://cmots.ca:8081/api/checkProductInfo";
...
void postReq()
{
  ...
  if (emailAddress.length() > 0 && IMEI.length() > 0)
  {
    ...
    if (WiFi.status() == WL_CONNECTED)
    {
      ...
      // Your Domain name with URL path or IP address with path
      http.begin(serverNameV);

      // Specify content-type header
      http.addHeader("Content-Type", "application/x-www-form-urlencoded");
      // Data to send with HTTP POST
      String httpRequestData = "imei=" + String(IMEI) + String("&email=") + String(emailAddress);
